...
public class TestClient {
	...
	public final void testMain() throws InterruptedException {
		 ...
		 * 
		 * String AGENT_PATH = "/org/bluez/agent"; Agent1 agent = new
		 * MyAgent(AGENT_PATH);
		 * 
		 * System.out.println(" ==> Agent1: " + agent); Thread.sleep(2000);
		 * System.out.println(" ==> Agent1: " + agent +
		 * " name aquired deathline -----");
		 */
		...
		for (Adapter1 a : adapters) {
			System.out.println(" ... adapter " + a.getObjectPath() + "  "
					+ a.getName());
			for (Device1 d : a.getDevices()) {
				System.out.println("  .. device " + d.getObjectPath() + "  "
						+ d.getName());
				for (GattService1 s : d.getServices()) {
					System.out.println("   . service " + s.getObjectPath()
							+ "  " + s.getUUID());
					for (GattCharacteristic1 c : s.getCharacteristics()) {
						System.out.println("    . char " + c.getObjectPath()
								+ "  " + c.getUUID());
					}
				}
			}
	...
	private static ObjectManagerSignalListener createSignalListener() {
		return new ObjectManagerSignalListener() {
			...
			@Override
			public void interfaceRemoved(ObjectManager m, String objectpath,
					String interfacename) {
				if (ObjectManager.getLogger().isLoggable(Level.FINER))
					System.out.println("interfaceRemoved path=" + objectpath
							+ " interface=" + interfacename);
			}

			@Override
			public void interfaceAdded(ObjectManager m, String objectpath,
					String interfacename) {
				if (ObjectManager.getLogger().isLoggable(Level.FINER))
					System.out.println("interfaceAdded path=" + objectpath
							+ " interface=" + interfacename);
			}
		};
	}
	...
	private static final class MyAgent extends Agent1CB {
		 ...
		 * agent. An agent can use it to do cleanup tasks. There is no need to
		 * unregister the agent, because when this method gets called it has
		 ...
		 * 1-16 characters length. The string can be alphanumeric. Possible
		 * errors: org.bluez.Error.Rejected org.bluez.Error.Canceled
		 */
		@Override
		 ...
		 * pincode for an authentication. An empty reply should be returned.
		 * When the pincode needs no longer to be displayed, the Cancel method
		 * of the agent will be called. This is used during the pairing process
		 * of keyboards that don't support Bluetooth 2.1 Secure Simple Pairing,
		 * in contrast to DisplayPasskey which is used for those that do. This
		 ...
		 * later specification. Possible errors: org.bluez.Error.Rejected
		 * org.bluez.Error.Canceled
		 */
		@Override
		public void displayPinCode(Object device, String pincode) {
			System.out
					.println("Agent1 callback displayPinCode() -> " + pincode);
		}
		 ...
		 *
		 * This method gets called when the service daemon needs to get the
		 * passkey for an authentication. The return value should be a numeric
		 * value between 0-999999. Possible errors: org.bluez.Error.Rejected
		 * org.bluez.Error.Canceled
		 */
		@Override
		 ...
		 * number of already typed keys on the remote side. An empty reply
		 * should be returned. When the passkey needs no longer to be displayed,
		 * the Cancel method of the agent will be called. During the pairing
		 * process this method might be called multiple times to update the
		 * entered value. Note that the passkey will always be a 6-digit number,
		 * so the display should be zero-padded at the start if the value
		 * contains less than 6 digits.
		 */
		@Override
		public void displayPasskey(Object device, int passkey, short entered) {
			System.out.println("Agent1 callback displayPasskey() -> " + passkey
					+ " " + entered);
		}
		...
		public void requestConfirmation(Object device, int passkey) {
			System.out.println("Agent1 callback requestConfirmation() -> "
					+ passkey);
		}
		 ...
		 * just-works model. Possible errors: org.bluez.Error.Rejected
		 * org.bluez.Error.Canceled
		 */
		@Override
		...
		public void authorizeService(Object device, String uuid) {
			System.out
					.println("Agent1 callback authorizeService() --> " + uuid);
		}
